...
# Get path from context menu/Send to/ .cmd
path_folder_to_upload=''.join(sys.argv[1:])
path_folder_to_upload1= path_folder_to_upload.replace("C:", "|C:")	
lst_of_path= path_folder_to_upload1.split("|")
del lst_of_path[0]

FULL_PATH = lst_of_path[0]
...
def folder_upload(service):
	...
	gdrive_folders_id_dico = {}

	for root, _, files in os.walk(FULL_PATH, topdown=True):
		last_dir = root.split('\\')[-1] 
		pre_last_dir = root.split('\\')[-2]
		if pre_last_dir not in gdrive_folders_id_dico.keys():
			pre_last_dir = []
		else:
			pre_last_dir = gdrive_folders_id_dico[pre_last_dir]
		...
		gdrive_folders_id_dico[last_dir] = folder_id

	return gdrive_folders_id_dico
...
def check_upload(service):
	...
	if DIR_NAME in [item['name'] for item in items]:
		...
		gdrive_folders_id_dico = folder_upload(service)
		folder_id = gdrive_folders_id_dico[DIR_NAME]
...
def get_tree(folder_name, gdrive_folders_tree_list, root, gdrive_folders_id_dico, service):
	...
	Args:
		...
		gdrive_folders_tree_list: List of relative folder paths, initially
		empy list.
		root: Current relative folder path, initially empty string.
		gdrive_folders_id_dico: Dictionary with pairs of {key:value} like
	...
	"""
	folder_id = gdrive_folders_id_dico[folder_name]
	...
	for item in items:
		gdrive_folders_id_dico[item['name']] = item['id']
		gdrive_folders_tree_list.append(root + item['name'])
		folder_id = [i['id'] for i in items
					if i['name'] == item['name']][0]
		folder_name = item['name']
		get_tree(folder_name, gdrive_folders_tree_list,
				root, gdrive_folders_id_dico, service)
...
def main():
	...
	gdrive_folders_tree_list = []
	root = ''
	gdrive_folders_id_dico = {}

	gdrive_folders_id_dico[folder_name] = folder_id
	get_tree(folder_name, gdrive_folders_tree_list, root, gdrive_folders_id_dico, service)
	local_folders_tree_list = []
	...
	for root, dirs, files in os.walk(full_path, topdown=True):
		for name in dirs:
			...
			local_folders_tree_list.append(os.path.join(var_path, name))
	print("Structures of folders (Gdrive + local)")
	print("gdrive_folders_id_dico", gdrive_folders_id_dico)
	print("gdrive_folders_tree_list", gdrive_folders_tree_list)
	print("local_folders_tree_list", local_folders_tree_list)
	print("\n")
	
	# old folders on drive
	gdrive_folders_to_be_removed = list(set(gdrive_folders_tree_list).difference(set(local_folders_tree_list)))
	# new folders on drive, which you dont have(i suppose hehe)
	new_folders_to_be_uploaded = list(set(local_folders_tree_list).difference(set(gdrive_folders_tree_list)))
	# foldes that match
	existing_folders_to_be_updated = list(set(local_folders_tree_list).intersection(set(gdrive_folders_tree_list)))
	print("Changes to make")
	print("gdrive_folders_to_be_removed",gdrive_folders_to_be_removed)
	print("new_folders_to_be_uploaded", new_folders_to_be_uploaded)
	print("existing_folders_to_be_updated", existing_folders_to_be_updated)
	print("\n")
	# Add starting directory
	existing_folders_to_be_updated.append(folder_name)
	# Sort uploadable folders
	# so now in can be upload from top to down of tree
	new_folders_to_be_uploaded = sorted(new_folders_to_be_uploaded, key=by_lines)

	# Here we upload new (abcent on Drive) folders
	for folder_dir in new_folders_to_be_uploaded:
		root_of_folder_to_upload = (os.path.sep).join(full_path.split(os.path.sep)[0:-1]) + os.path.sep
		path_of_folder_to_upload = root_of_folder_to_upload + folder_dir 
		last_dir = folder_dir.split(os.path.sep)[-1]
		pre_last_dir = folder_dir.split(os.path.sep)[-2]


		files = [f for f in os.listdir(path_of_folder_to_upload)
				if os.path.isfile(os.path.join(path_of_folder_to_upload, f))]

		folder_metadata = {'name': last_dir,
						'parents': [gdrive_folders_id_dico[pre_last_dir]],
						'mimeType': 'application/vnd.google-apps.folder'}
		...
		for os_file in files:
			...
			os_file_mimetype = mimetypes.MimeTypes().guess_type(
				os.path.join(path_of_folder_to_upload, os_file))[0]
			media = MediaFileUpload(os.path.join(path_of_folder_to_upload, os_file),
	...
	for folder_dir in existing_folders_to_be_updated:

		root_of_folder_to_upload = (os.path.sep).join(full_path.split(
			os.path.sep)[0:-1]) + os.path.sep

		path_of_folder_to_upload = root_of_folder_to_upload + folder_dir
		last_dir = folder_dir.split(os.path.sep)[-1]
		# print(last_dir, folder_dir)
		os_files = [f for f in os.listdir(path_of_folder_to_upload)
					if os.path.isfile(os.path.join(path_of_folder_to_upload, f))]
		results = service.files().list(
			pageSize=1000, q=('%r in parents and \
			mimeType!="application/vnd.google-apps.folder" and \
			trashed != True' % gdrive_folders_id_dico[last_dir]),
		...
		for drive_file in refresh_files:
			file_dir = os.path.join(path_of_folder_to_upload, drive_file['name'])
			...
			if (file_time > drive_time) or (drive_md5 != os_file_md5):
				...
				file_metadata = {'name': drive_file['name'],
								'parents': [gdrive_folders_id_dico[last_dir]]}
		...
		for os_file in upload_files:
			...
			file_dir = os.path.join(path_of_folder_to_upload, os_file)
	...
	gdrive_folders_to_be_removed = sorted(gdrive_folders_to_be_removed, key=by_lines, reverse=True)
	
	# Delete old folders from Drive
	for folder_dir in gdrive_folders_to_be_removed:
		...
		folder_id = gdrive_folders_id_dico[last_dir]
		service.files().delete(fileId=folder_id).execute()

if __name__ == '__main__':
	main()
	print("Local folder uploaded/sync to Gdrive : done ! ")
